...
1. **認証**：`APP_PASSWORD` を必ず設定。未設定だと**誰でも生放送のテロップを操作**できてしまいます。
...
| 変数 | 必須 | 説明 |
| --- | --- | --- |
| `APP_PASSWORD` | ◎(公開時) | 操作画面のログインパスワード |
| `OUTPUT_TOKEN` | 任意 | 設定すると出力画面/WSもトークン必須に（完全非公開化）。vMix URLは `…/output?token=値` |
